	...
	<div class="article">
		<card>
			<Form :model="formItem" :label-width="80">
				...
				<FormItem label="上传图文">
					<div v-for="(item, index) in formItem.pictureDetails" :key="index">
						<Row style="margin-bottom: 10px;">
							<Col span="2">
								<Avatar shape="square" icon="ios-person" :src="item.picutreUrl" size="large" />
							</Col>
							<Col span="16">
								<Input  v-model="item.content" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入图片说明"></Input>
							</Col>
							<Col span="2" offset="1">
...
<script>
	...
	import pictureApi from '../assets/api/picture.js'
	...
	export default {
		...
		methods: {
			...
			handleSuccess2(res){
				this.formItem.pictureDetails.push({
					content: '',
					picutreUrl: res.data.url
				})
			},
			atrcleRemove(index){
				this.formItem.pictureDetails.splice(index, 1)
			},
			handleSubmit(){
				if(tools.check(this.formItem)){
					...
					par['pictureDetails'] = JSON.stringify(par['pictureDetails'])
					this.$http.rq({
						url: pictureApi.picture,
						method: 'post',
						data: par
					}).then(res =>{
						if(res.code === 200){
